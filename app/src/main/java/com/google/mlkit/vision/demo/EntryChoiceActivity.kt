...
class EntryChoiceActivity : AppCompatActivity(), ActivityCompat.OnRequestPermissionsResultCallback {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    var signatureView :TextView = this@EntryChoiceActivity.findViewById(R.id.signature) as TextView
    signatureView.setOnLongClickListener {
//this does not show!            Toast.makeText(this, "bye, come back soon !", Toast.LENGTH_LONG).show()
      this.finish()
      exitProcess(0)
    }
...
//    val buildDate = dateFormat.format(Date(BuildConfig.BUILD_DATE))
    signatureView.text = "%s esLabPostureCoach, crafted by raxy".format( dateFormat.format(Date(BuildConfig.BUILD_DATE)))
