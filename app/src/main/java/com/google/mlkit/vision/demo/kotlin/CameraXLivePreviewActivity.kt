...
import com.google.mlkit.vision.demo.kotlin.posedetector.SkeletalJoint
...
class CameraXLivePreviewActivity :
  ...
  private var lensFacing = CameraSelector.LENS_FACING_FRONT // IVN!
  ...
  override fun onItemSelected(parent: AdapterView<*>?, view: View?, pos: Int, id: Long) {
    ...
    if( pos == 0) { // IVN disable all but pose detection!
      bindAnalysisUseCase()
    } else {
      Toast.makeText(applicationContext, "Sorry, only pose detection is enabled!", Toast.LENGTH_SHORT).show()
    }
  }
      ...
      try {
        when (selectedModel) {
          ...
          POSE_DETECTION -> {
            ...
            val singleLandmarkSideIsRight =
              PreferenceUtils.singleLandmarkSideIsRight(this)
